...
function setTransfromOrigin(node: any, value: string) {
  const style = node.style
  let browserPrefix = ['Webkit', 'Moz', 'Ms', 'ms']
  browserPrefix.forEach((prefix: string) => {
    ...
    if (visible) {
      if (!prevProps.visible) {
        ...
        if (mousePosition) {
          ...
          setTransfromOrigin(dialogNode, `${mousePosition.x - elOffset.left}px ${mousePosition.y - elOffset.top}px`)
        } else {
          setTransfromOrigin(dialogNode, '')
        }
      }
  ...
  getDialogElement = () => {
    ...
    return (
      <Animate
        ...
        transitionAppear
      >
        {this.props.visible || !props.destroyOnClose ? dialogElement : null}
      </Animate>
    )
  }
    ...
    return (
      <div className={`${prefixCls}-root`}>
        ...
        <div
          ...
          onMouseUp={maskClosable ? this.onMaskMouseUp : undefined}
          role="dialog"
          aria-labelledby={props.title ? this.titleId : undefined}
          style={style}
          {...props.wrapProps}
        >
